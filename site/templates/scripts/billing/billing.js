...
$('#ccno').validateCreditCard(function(result) {
	if (result.card_type !== null && $("#ccno").val().length > 0) {
		if ($("#ccno").val().length >= 3 && result.length_valid === false) {
		...
		}  else if ($("#ccno").val().length >= 10 && result.luhn_valid === false) {
			$("#creditcard-num").removeClass("has-success").addClass("has-error");
			$('.payment-errors').text('Please check your Credit Card Number');
			$('.card').removeClass('hidden');
		} else if ($("#ccno").val().length === 0) {
			$("#creditcard-num").removeClass("has-success").addClass("has-error");
			$('.payment-errors').text('Please check your Credit Card Number');
			$('.card').removeClass('hidden');
		} else {
	...
	} else if (result.card_type === null && $("#ccno").val().length > 0 ) {
		$("#creditcard-num").removeClass("has-success").addClass("has-error");
...
$('input[name=cvc]').change(function() {
	var card = $.payment.cardType($("#ccno").val());
	console.log(card);
